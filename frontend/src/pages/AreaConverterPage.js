...
const AreaConverterPage = () => {
    ...
    const AreaVisualization = () => {
        // Responsive base size based on container width
        const [containerSize, setContainerSize] = useState(180);
        const containerRef = React.useRef(null);
        const baseSize = Math.max(60, Math.min(containerSize * 0.5, 180));
        const scaleFactor = Math.min(visualScale, 2);
        // Cap the max scale to prevent overflow
        const maxSquareSize = containerSize * 0.9;
        ...
        // Calculate actual size for each square
        const getSquareSize = (scale) => Math.min(baseSize * scale, maxSquareSize);

        return (
            <div ref={containerRef} className="flex flex-col items-center justify-center p-6 bg-gray-800 rounded-xl w-full max-w-xs mx-auto">
                <h3 className="text-lg font-bold text-brand-accent mb-4">Area Visualization</h3>
                <div className="relative flex items-center justify-center" style={{ minHeight: maxSquareSize }}>
                    {/* Square Feet (Base) */}
                    <motion.div
                        initial={{ scale: 0 }}
                        animate={{ scale: 1 }}
                        ...
                        style={{ width: getSquareSize(scaleFactor), height: getSquareSize(scaleFactor), zIndex: 3 }}
                    />
                    {/* Comparative sizes */}
                    <motion.div
                        initial={{ scale: 0 }}
                        ...
                        style={{ width: getSquareSize(scaleFactor * Math.sqrt(CONVERSIONS.sqmeter)), height: getSquareSize(scaleFactor * Math.sqrt(CONVERSIONS.sqmeter)), zIndex: 2 }}
                    />
                    <motion.div
                        initial={{ scale: 0 }}
                        animate={{ scale: 1 }}
                        ...
                        style={{ width: getSquareSize(scaleFactor * Math.sqrt(CONVERSIONS.sqyard)), height: getSquareSize(scaleFactor * Math.sqrt(CONVERSIONS.sqyard)), zIndex: 1 }}
                    />
                </div>
    ...
    return (
        <PageTransition>
            <div className="relative min-h-screen text-text-light px-4 py-12 overflow-hidden">
                ...
                <motion.h1 
                    ...
                    className="text-4xl font-extrabold text-white mb-2 text-center drop-shadow-lg"
                >
                    Area Unit Converter
                </motion.h1>
                <motion.p 
