...
const LineChart = ({ data, color = "#2563eb" }) => {
    ...
    return (
        <div className="relative h-64 w-full min-w-[18rem] max-w-[24rem] mx-auto flex flex-col justify-end p-2">
            {/* Y-axis grid lines and labels removed for a minimal look */}
            <div className="flex items-end justify-between h-48 w-full px-2 z-10">
                {data.map((height, index) => (
                    <motion.div
                        ...
                        className={`w-7 mx-1 rounded-t bg-blue-500 flex flex-col items-center justify-end shadow-sm`}
                        style={{ minHeight: '2rem' }}
                    >
                        <span className="text-xs text-gray-700 font-medium mb-2 animate-fade-in" style={{ marginBottom: '-1.5rem' }}>{height}</span>
                    </motion.div>
                ))}
            </div>
            {/* Chart title removed for cleaner look; section header provides context */}
        </div>
    );
};

const PieChart = ({ data, colors = ["#2563eb", "#10b981", "#f59e0b", "#6366f1"] }) => {
    // Pie chart math
    const total = data.reduce((a, b) => a + b, 0);
    let cumulative = 0;
    const radius = 40;
    const center = 50;
    const pieSegments = data.map((value, i) => {
    ...
    // Legend
    return (
        <div className="relative h-64 w-full min-w-[18rem] max-w-[24rem] mx-auto flex flex-col items-center justify-center p-2">
            <svg viewBox="0 0 100 100" className="w-44 h-44 z-10">
                {pieSegments}
            </svg>
            <div className="flex flex-row flex-nowrap justify-center items-center gap-3 mt-6 overflow-x-auto">
                {data.map((_, i) => (
                    <span key={i} className="flex items-center text-xs text-white/90 font-medium bg-white/20 backdrop-blur px-4 py-1 rounded-full shadow-sm border border-white/10 whitespace-nowrap">
                        <span className="inline-block w-3 h-3 rounded-full mr-2 align-middle" style={{ background: colors[i % colors.length] }}></span>
                        {`Segment ${i + 1}`}
                    </span>
                ))}
            </div>
